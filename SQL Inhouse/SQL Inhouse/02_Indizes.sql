...
dbcc showcontig('kundeumsatz') -- 40455

alter table kundeumsatz add id int identity

dbcc showcontig('kundeumsatz') -- 41092

set statistics io, time on
select * from kundeumsatz where id =  100 --  57210,  vs  41092
...
--Welcher Plan? -- T SCAN
select id from kundeumsatz where id = 100  --57206
--Tab Scan

--besser durch: NIX_ID  --IX SEEK
select id from kundeumsatz where id = 100  --3
...
select id, freight from kundeumsatz where id < 10500 --ab 11500 ca Table scan


--besser mit: NIX_ID_FR (zusammengesetzter IX)
select id, freight from kundeumsatz where id < 900500 --ab 10500 ca Table scan
...
create view vdemo
as
select country, count(*) as ANz from kundeumsatz
group by country
GO
...
as
select country, count_big(*) as ANz from dbo.kundeumsatz
group by country
GO
